...
### Feature Importance
-    Across both horizons, the most influential signal is the **current week wholesale price**, which is expected in short-horizon forecasting because recent price level acts as a strong anchor for near-term movements (the delta is typically smaller than the absolute level). The second major driver is **seasonality** and the cyclical encodings—indicating a consistent calendar pattern in weekly maize prices (e.g., harvest, storage, and end-of-year trading effects).  
